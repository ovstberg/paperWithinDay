...
The time-of-day dependent utility of starting an activ is given by linear interpolation between the closest defined parameters:
\begin{equation*}
...
\begin{aligned}
u_\text{stay}(t,\act{p}) &= \int_t^{t+\dura{p}} u_\text{stay,marg}(\tau,p) % \frac{\theta_{p,T_k}(T_{k+1}-\tau)+\theta_{p,T_{k+1}}(\tau-T_k) }{T_{k+1}-T_{k}}
